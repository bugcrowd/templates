# Token Leakage via Referrer Header over HTTP

## Overview

A malicious attacker can Man-in-The-Middle (MiTM) users to expose tokens in HTTP headers to escalate privileges and execute API calls within the application.
...
1. {{action}} and notice the exposed token in the Referrer Header and the request in over HTTP:
...
Sensitive data disclosure of sensitive tokens in the Referrer header allows attackers to MiTM users and execute API calls on behalf of the user.
