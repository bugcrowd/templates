# Sensitive Data Exposure of Sensitive Token in Local / Session Storage

## Overview

A malicious attacker can Man-in-The-Middle (MiTM) or exploit third-party vendors for exposed tokens in HTTP headers to escalate privileges and execute API calls within the vulnerable target.
...
1. {{action}} and notice the exposed token in the Referrer Header and the request is sent to a third-party vendor:
...
Sensitive data disclosure of sensitive tokens in the Referrer header allows attackers to MiTM users or exploit third parties to access the exposed tokens and execute API calls on behalf of the users in the application.
