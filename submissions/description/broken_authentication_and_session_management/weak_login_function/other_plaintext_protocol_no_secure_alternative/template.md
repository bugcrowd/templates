# Other Plaintext Protocol Does Not Implement Secure Alternative

## Overview
Weak login function on {{value}} protocol allows a malicious attacker to Man-In-The-Middle (MiTM) requests to sniff critical system or user information.
...
1. The {{value}} protocol on port {{value}} is accessible using {{software}}:

{{screenshot}}

1. {{action}} to interact with the weak login function
...
A malicious attacker can expliot the vulnerable protocol to {{action}}.
